...
def load_master_taban(path="master_taban.csv"):
    expected_counts = {}
...
def load_master_items(path="master_items.csv"):
    allowed_items = []
...
def load_master_work(path="master_work.csv"):
    work_master = {}
...
def load_master_spec(path="master_spec.csv"):
    spec_master = {}
...
def run_ocr(image_folder, output_folder, log):
    ...
    expected_counts = load_master_taban()
    allowed_items = load_master_items()
    work_master = load_master_work()
    spec_master = load_master_spec()

    log("田番マスタ読込: " + str(expected_counts))
    log("測定項目マスタ読込: " + ", ".join(allowed_items))
    log("工種マスタ読込: " + str(work_master))
    log("規格値マスタ読込: " + str(spec_master))
...
class SSKApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("SSK写真 OCR → デキスパートCSV")
        self.root.geometry("900x650")

        self.image_folder_var = tk.StringVar(value="hhh")
        self.output_folder_var = tk.StringVar(value="zzz")
    ...
    def build_ui(self):
        ...
        self.run_button.pack(fill="x", padx=15, pady=10)
    ...
    def run_ocr_thread(self, image_folder, output_folder):
        try:
            run_ocr(
                image_folder,
                output_folder,
