...
MASTER_WORK_CSV = "master_work.csv"
...
def load_work_type_map(master_csv):
    """工種マスタCSVから、測定項目ごとの工種を読み込む。"""
    work_type_map = {}

    if not os.path.exists(master_csv):
        raise FileNotFoundError(f"工種マスタがありません: {master_csv}")
    ...
    with open(master_csv, "r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            item = str(row.get("測定項目", "")).strip()
            work_type = str(row.get("工種", "")).strip()
            ...
            work_type_map[item] = work_type

    return work_type_map
...
def run_ocr(image_folder, output_folder):
    ...
    work_type_map = load_work_type_map(MASTER_WORK_CSV)
    ...
    for index, image_name in enumerate(image_files, start=1):
        ...
        # 工種はGPTの読み取りより、マスタを優先する
        if measurement_item in work_type_map:
            work_type = work_type_map[measurement_item]
        elif work_type == "":
            work_type = "整地工"
